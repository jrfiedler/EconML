...
class _RLearner(LinearCateEstimator):
    ...
    n_splits: int, or a custom cross validator object
        The number of splits to use when fitting the first-stage models.  If this is an integer,
        then splits will be generated by `StratifiedKFold(shuffle=True)` with that number of splits
        if treatment is discrete and `KFold(shuffle=True)` otherwise.  Otherwise, the argument will
        be treated as a custom cross validator.
    ...
    def fit_nuisances(self, Y, T, X, W, sample_weight=None):
        if isinstance(self._n_splits, int):
            if self._discrete_treatment:
                splitter = StratifiedKFold(self._n_splits, shuffle=True, random_state=self._random_state)
            else:
                splitter = KFold(self._n_splits, shuffle=True, random_state=self._random_state)
        else:
            splitter = self._n_splits
